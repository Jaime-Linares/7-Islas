export default function PlayerEditChecks(playerDTO,player) {
    ...
    if (playerDTO.password !== player.user.password && (playerDTO.password.length < 7 || playerDTO.password.length > 15)) {
        message="La contraseña debe tener entre 7 y 15 caracteres";
        return message; 
    }

    if (playerDTO.password !== player.user.password && !/[!@#$%^&*(),.?":{}|<>]/.test(playerDTO.password)) {
